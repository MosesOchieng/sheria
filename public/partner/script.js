...
// Pause keyframes animations
setTimeout(function () {
	document.body.classList.remove('keyframes-hold');
}, 500);
...
const enableDarkMode = () => {
	...
	darkModeToggleAlt.checked = false;
	lightModeToggleAlt.checked = true;
};

const disableDarkMode = () => {
	document.body.classList.remove('light-mode');
	localStorage.setItem('darkMode', null);
	darkModeToggleAlt.checked = true;
	lightModeToggleAlt.checked = false;
};
...
darkModeToggleAlt.addEventListener('change', () => {
	this.checked = true;
	document.body.classList.remove('light-mode');
	localStorage.setItem('darkMode', null);
...
lightModeToggleAlt.addEventListener('change', () => {
	this.checked = true;
	document.body.classList.add('light-mode');
	localStorage.setItem('darkMode', 'enabled');
	darkModeToggle.checked = true;
...
const passwordToggle = document.querySelector('.password-view');
...
tagInput1.addData(['JavaScript', 'CSS']);
...
function activeHightLightLink() {
	const activeTagCords = activeItem.getBoundingClientRect();
	const activeCoords = {
		width: activeTagCords.width,
		// height: activeTagCords.height,
		top: (activeTagCords.top = 0),
		left: activeTagCords.left + window.screenX,
	};

	hightLight.style.width = `${activeCoords.width}px`;
	// hightLight.style.height = `${activeCoords.height}px`;
	hightLight.style.transform = `translate(${activeCoords.left}px, ${activeCoords.top}px)`;
}

window.onload = function () {
	setTimeout(function () {
		activeHightLightLink();
	}, 300);
};
...
document.getElementById('selectAll').onclick = function () {
	var checkboxes = document.getElementsByName('selectAllItem');
	for (var checkbox of checkboxes) {
		checkbox.checked = this.checked;
		overall.parentNode.classList.remove('indeterminate-active');
	}

	if (textSwap.innerHTML === 'Select Group') {
		textSwap.innerHTML = 'Group Selected';
	} else {
		textSwap.innerHTML = 'Select Group';
	}
...
const lastName = fullName.innerHTML;
const [first, last] = lastName.split(' ');

fullName.innerHTML = `${first} ${last.substring(0, 1)}.`;